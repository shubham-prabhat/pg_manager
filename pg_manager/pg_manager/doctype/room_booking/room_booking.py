    ...
    if doc.docstatus == 1:
        ...
        for room_entry in doc.room_table:
            ...
            if current_status != "Occupied":
                ...
                frappe.db.set_value("Room", room_entry.room, "customer_identity_number", doc.customer_identity_number)
...
def on_cancel(doc, method):
    ...
    if doc.docstatus == 2:
        # Check the customer_identity_number condition
        if doc.customer_identity_number:
            # Iterate through the rows in the room_table
            for room_entry in doc.room_table:
                # Check if the room status is Occupied
                room_status = frappe.get_value("Room", room_entry.room, "room_status")
                if room_status == "Occupied":
                    # Get the customer_identity_number from the 'Room' document
                    customer_identity_number = frappe.get_value("Room", room_entry.room, "customer_identity_number")
                    # Get the customer_contact_number from the 'Room' document
                    customer_contact_number = frappe.get_value("Room", room_entry.room, "customer_contact_number")
                    
                    # Check if the customer_identity_number and customer_contact_number match
                    if doc.customer_identity_number != customer_identity_number or doc.customer_contact_number != customer_contact_number:
                        frappe.throw("Cancellation not allowed. Customer identity number or contact number does not match.")
                    ...
                    frappe.db.set_value("Room", room_entry.room, "customer_identity_number", None)
            ...
            frappe.throw("Cancellation not allowed. Customer identity number is required.")
